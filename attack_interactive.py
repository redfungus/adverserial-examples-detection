...
#         print('adversarial:', classes_list[pred2])
        return [input_str, '1' if classes_list[pred1] == 'Positive' else '0', adv_str, '1' if classes_list[pred2] == 'Positive' else '0', '1' if classes_list[pred1] == classes_list[pred2] else '0']
    elif mode=='char':
...
if __name__ == '__main__':
    ...
    data = pd.read_csv('dataset.csv', encoding = "ISO-8859-1", header = 0) 
    ...
    with open("output.csv", "w", newline="") as f:
        ...
        for index, row in data.iterrows():
            line = visualize(row['SentimentText'], power=10, mode=args.datatype, model = model, dict_word = word_index, index2word = index2word, classes_list = classes_list, device = device)
#             print (line)
    ...
    while s:
        visualize(s, power=10, mode=args.datatype, model = model, dict_word = word_index, index2word = index2word, classes_list = classes_list, device = device)
